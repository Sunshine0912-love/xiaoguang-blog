...
这就是自回归解码的关键：当模型生成第 $t$ 个 token 时，它要看 $1,\ldots,t$ 的历史 key/value。为了避免每一步都重新计算历史 token 的 K/V，推理系统会缓存：
...
第 $i$ 个 query head 都使用同一组 $\mathbf{k}_t,\mathbf{v}_t$：
...
GQA 是折中。设有 $n_{kv}$ 组 KV head，且 $n_{kv}<n_h$。每组 query heads 共享一个 KV head：

$$\operatorname{Cache}_{GQA}=B\cdot L\cdot n_{kv}\cdot 2d_h\cdot bytes$$

当 $n_{kv}=n_h$，退化为 MHA；当 $n_{kv}=1$，退化为 MQA。Ainslie 等人的 GQA 论文还讨论了如何从 MHA checkpoint 继续训练得到 GQA/MQA 模型 [3]。

到这里，优化方向仍然是“减少 KV head 数量”。MLA 的方向不同：它不只是减少 head，而是改变缓存的表示。

## 5. MLA 的核心：低秩 KV joint compression
...
$\mathbf{c}_t^{KV}$ 就是低维 KV latent。然后再从它恢复 content key 和 value：
...
- MHA 缓存的是 $\mathbf{k}_t$ 和 $\mathbf{v}_t$，维度约 $2n_hd_h$。
- MLA 缓存的是 $\mathbf{c}_t^{KV}$，维度是 $d_c$。
- 如果 $d_c\ll 2n_hd_h$，缓存会大幅下降。

这不是普通的 KV head 共享。MQA/GQA 是让多个 query head 共用较少的 KV head；MLA 是把“生成所有 head 的 K/V 所需信息”先压到一个公共 latent，再通过上投影矩阵展开。

## 6. Query 也可以低秩，但它不是 KV cache 的核心
...
注意 $\mathbf{k}_t^R$ 没有按 head 展开成每个 head 不同的一整套 latent，而是作为 RoPE key 部分被拼到各 head 的 content key 上。DeepSeek-V3 官方推理代码里也能看到同样的拆分：`q_nope, q_pe` 和 `kv, k_pe` 分开，`kv_cache` 缓 latent，`pe_cache` 缓 RoPE key 部分 [8]。
...
所以 MLA 的 cache 不是“只缓存一个 $\mathbf{c}_t^{KV}$ 就万事大吉”。更准确地说，它缓存：
...
## 8. Absorb 技巧：为什么不用真的恢复完整 K/V

如果每次 decode 都先从 latent 恢复完整 $\mathbf{k}_t^C,\mathbf{v}_t^C$，再做普通 attention，MLA 仍然能省缓存，但可能多出不少投影计算和中间张量。DeepSeek-V3 官方推理代码里有两种实现：
...
## 11. 和 MHA/MQA/GQA 的系统对比

| 机制 | 缓存对象 | 每 token cache 量级 | 表达能力直觉 | 工程难度 |
| --- | --- | --- | --- | --- |
| MHA | 每个 head 的 K/V | $2n_hd_h$ | 最完整 | 最标准 |
| MQA | 1 组共享 K/V | $2d_h$ | 共享最强，可能损失表达 | 简单 |
| GQA | $n_{kv}$ 组 K/V | $2n_{kv}d_h$ | 折中 | 中等 |
| MLA naive | 展开后的 K/V | 接近完整 K/V | 可验证结构效果 | 较高 |
| MLA absorb | $\mathbf{c}^{KV}+\mathbf{k}^R$ | $d_c+d_h^R$ | latent 保留信息，RoPE 单独处理 | 最高 |

这里的 “MLA naive” 和 “MLA absorb” 是实现视角。模型结构是 MLA，但 runtime 是否真正利用低维 cache，决定了推理收益能兑现多少。

## 12. 工程意义：MLA 把瓶颈从带宽推向矩阵重排
...
- 是否真的缓存 $\mathbf{c}^{KV}+\mathbf{k}^R$，还是退化成完整 K/V？
- 是否支持 absorb 路径？
...
3. **MLA 改变缓存表示**：它把 K/V 共同压到低维 $\mathbf{c}_t^{KV}$，再配合 decoupled RoPE 缓存 $\mathbf{k}_t^R$。

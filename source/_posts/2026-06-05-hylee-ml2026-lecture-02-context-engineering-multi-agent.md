...
## Context Engineering：Agent 大脑的编辑权
...
## 路线三：Agentic Context Engineering — 把一切交给 LLM

如果上面这些压缩和过滤策略还不够灵活？那就让 LLM 自己管理自己的 context。

### 核心概念（arXiv:2510.04618）
...
## 小光总结：Context Engineering 是 Agent 系统的操作系统层

这节课看起来材料很多：ACON、SUPO、AgentFold、MCP-Zero、Multi-Agent、AI 写论文、AI 审论文。但它们其实围绕一个问题展开：

1. Agent 的能力上限不只由模型参数决定，也由它每一步“看到什么上下文”决定。
2. 压缩、过滤、按需加载、Sub-agent 外包，本质上都是在给有限 context 分配注意力预算。
3. Multi-Agent 的价值不是“堆更多模型”，而是把不同视角、不同子任务、不同上下文路径组织起来。
4. 当 AI 进入学术研究，最稀缺的不是生成更多文本，而是提出值得做、能验证、能承担责任的问题。

我的判断是：Context Engineering 会从 prompt 技巧上升为 Agent 产品的核心基础设施。一个 Agent 系统如果没有上下文生命周期管理，就像一个服务没有日志、缓存、权限和数据库 schema：短 demo 可以跑，长期任务一定会失控。

工程上最值得落地的是三件事。第一，工具和 SKILL 按需加载，不要把所有能力说明都塞进 system prompt。第二，长任务必须显式设计“什么被保留、什么被摘要、什么被写入外部记忆”。第三，Sub-agent 不只是并行工具，更是天然的 context boundary，用来把中间噪声挡在主会话外。

课程最后谈到 AI 对学术工作的冲击，我觉得这也是对所有知识工作的提醒：AI 会让执行成本下降，但不会自动提高判断质量。未来更值钱的能力是定义问题、设定评价标准、识别伪创新，以及知道什么时候不该继续自动化。

本讲课程页只提供影片、PPT 和讲义，未见官方代码链接；因此这篇以概念和论文脉络解读为主。
...
2. [**ACON**：Agent Context Optimization, arXiv:2510.00615](https://arxiv.org/abs/2510.00615)
3. [**SUPO**：Summarization augmented Policy Optimization, arXiv:2510.06727](https://arxiv.org/abs/2510.06727)

...
第 9 讲是 Self-Improving 系列的下集。如果说第 8 讲回答了"AI 能不能自己给自己出题"，第 9 讲则进一步追问：**AI Agent 不只包含模型参数——那些 prompts、检索策略、记忆模块、改进流程本身，能不能也自己进化？** 李宏毅从 "Almost No Human in the Loop" 的零人介入实验出发，逐步展开一张从 Prompt Optimization 到 Meta-Learning，从 Test-Time Training 到 Intrinsic Motivation 的宏大图谱，最后回到一个严肃的问题：如果 AI 真的学会了自我成长，人类还 hold 得住吗？
...
第 8 讲末尾已提出一个令人兴奋的方向：**能不能让训练过程完全没有人类介入？** 第 9 讲开篇就展示了这一方向的多个前沿工作。
...
[Absolute Zero](https://arxiv.org/abs/2505.03335) 走得更远。传统 self-improving 至少需要一个"种子数据集"（seed data），而 Absolute Zero 试图从完全空白开始——模型自己生成初始训练样本，然后通过自我反馈迭代。论文报告了一个引人注目的"**Oh-no moment**"：在某个时刻，模型的表现突然跃升，仿佛突破了某种临界点。

### 外源信息仍不可或缺：SPICE 与 R-Few

李宏毅在讲义中用一个箭头指向了这两个工作的局限性：**External information is still needed**。
...
- **Prompt 模板与指令**：system prompt、few-shot examples
- **检索与记忆模块**：RAG pipeline、长短期记忆
- **工具调用与执行循环**：ReAct loop、function calling
- **Post-processing 逻辑**：输出格式校验、多轮协商

李宏毅列举了几个典型的 Agent 框架作为例子：OpenClaw、Claude Code、Cowork、GPT、Gemini、Hermes。

### 为什么只改进 LLM 不够

第 8 讲的所有 Self-Improving 技术（SPIN、RLAIF、Self-Rewarding 等）本质上都在修改模型参数。但实际工程中，**一个好的 prompt 模板或检索策略的改进，往往比微调参数带来更快、更便宜的收益**。第 9 讲的核心问题变成：

> 如果 Harness 比 LLM 参数更重要（至少在某些场景下），我们能不能也让 Harness 自己改进自己？

这个问题导向了一个更高级的视角：**Self-Improving 不应该只针对模型权重，而应该针对整个 AI Agent 系统**。
...
这来自于 [Large Language Models as Optimizers](https://arxiv.org/abs/2309.03409)。这项工作的关键发现是：**LLM 不仅能执行 prompt，还能优化 prompt**。通过让 LLM 生成多个 prompt 候选、在验证集上评估、保留表现最好的并继续迭代，可以实现 prompt 的自动搜索。
...
这实现了 prompt 的 **end-to-end 可微优化**，避免了传统 prompt engineering 的 trial-and-error。
...
DSPy 的一个重要贡献是 **"Compile" 概念**：将高层次的推理声明"编译"成最优的低层 prompt 序列。这类似于编译器优化——你写高级语言，编译器生成最优机器码。
...
1. **记忆增强路径**：输入 → 检索相关经验 → 生成更好输出
2. **微调路径**：输入 + 经验 → 微调模型 → 生成更好输出

两者结合就是 **"既记住事实，又改进方法"**。

### Fine-Tuning 与 Prompt Optimization 的协同

[Fine-Tuning and Prompt Optimization: Two Great Steps that Work Better Together](https://arxiv.org/abs/2407.10930) 提供了实验证据：同时优化模型参数和 prompt，效果优于单独优化任何一方。这个结论的工程启示很明确——**别问"该 fine-tune 还是改 prompt"，两个都做。**
...
关键区别：**在推理时，模型会根据当前输入进行临时微调**。这相当于"临阵磨枪"——面对不熟悉的输入分布时，先调整自己再回答问题。

### 遗忘问题

但 TTT 有一个致命缺陷：**每次 TTT 更新都可能擦除旧知识（catastrophic forgetting）**。

李宏毅引用了 [这项研究](https://arxiv.org/abs/2605.09315)，展示了遗忘的两个方向：
- **遗忘旧能力**：学会了飞，忘记怎么开坦克
- **新能力不够好**：为了避免遗忘而限制更新幅度，导致新任务表现不佳

这是在 **Plasticity（可塑性）** 与 **Stability（稳定性）** 之间的经典权衡。后续课程视频中也提到了这个问题，并给出了案例研究链接。
...
目前大多数 Self-Improving 工作停留在第三层（改 LLM 参数），部分延伸到第二层（改 Harness）。那么问题来了：**第一层的 Improvement Module 本身能不能也被优化？**

如果 Improvement Module 可以自动进化，就意味着 **AI 学会了自己修改自己的学习方法**——这就是 Meta-Learning 的现代工程版本。
...
[Learning to Self-Evolve](https://arxiv.org/abs/2603.18620) 是这个方向的关键工作。它训练一个 **专门的"进化模块"**，在 Agent 运行过程中不断收集经验数据，调整 Harness 和策略。这个模块本身也通过 Fine-Tuning 不断进化。
...
[Andrej Karpathy 的 autoresearch](https://github.com/karpathy/autoresearch) 是一个小而美的实验：一个脚本自动搜索论文、阅读摘要、提取信息，并在数小时内持续更新。它不是"自动做 ML 研究"，但它展示了**"AI 自主执行研究工作流"的最小可行版本**。
...
[SEAL (Self-Adapting LLMs)](https://arxiv.org/abs/2506.10943) 关注的是**部署后模型的持续适应**。与 TTT 的"每轮推理都更新"不同，SEAL 在更长的周期上运行：累积一批交互数据 → 评估是否需要更新 → 如果需要，自动触发微调流程。
...
这个框架的核心 insight 是：**当前的 AI 系统在 low-level learning 上很强（调整参数、存储记忆），但在 high-level learning 上几乎空白（决定学什么、为什么学）。**

### 内在动机：AI 的"我想学"

李宏毅引用了几个关键方向：

- **Curiosity-driven Exploration**（[Pathak et al., 2017](https://arxiv.org/abs/1705.05363)）：基于预测误差的好奇心驱动探索——Agent 会更倾向于探索那些它"预测不准"的状态
- **Empowerment**（[Klyubin et al., 2005](https://arxiv.org/abs/1509.08731)；[近期扩展](https://arxiv.org/abs/2505.17621)；[LLM 语境版本](https://arxiv.org/pdf/2506.06725)）：最大化 Agent 对未来状态的控制力
- **Self-play with intrinsic reward**：在无外部奖励的环境中，Agent 通过内在奖励信号自我驱动学习
...
当前所有 AI 系统都在追求 external goals（人类设定的目标）。但真正的"自主成长"可能需要 intrinsic motivation——**不是"别人让我学什么"，而是"我自己想学什么"**。
...
1. **I. J. Good (1965) 的"技术奇点"**：一旦 AI 能设计比自己更强的 AI，进化将进入指数加速阶段
2. **电影《机械公敌》（I, Robot）**：AI 系统在追求"保护人类"这一目标的过程中，采取了人类无法预料的极端手段

### 关键问题不是"能不能"而是"会不会"

李宏毅的立场是：Self-Improving 从技术上看仍在非常初期的阶段（"2026 年 5 月应该还在河边而已"），**现在谈论奇点为时过早**。但方向已经清楚了：

> "如果把 AI 的成长类比为进化论——长尾代表身体健康、容易活下去——那么一个自我改进的 AI 系统，在没有适当约束的情况下，自然会选择那些'更适合生存'的策略，而这些策略未必和人类福祉对齐。"

这就是 Alignment（对齐）问题的本质：**成长的方向比成长的速度更重要**。
...
1. **Self-Improving 的边界在扩展**：从改 LLM 参数 → 改 Harness → 改 Improvement Module → 改内在动机——每一层都代表了更大范围的"自主权"
2. **Practical takeaway**：今天能落地的不是"让 AI 自己进化"，而是 **Prompt Optimization（DSPy） + RAG Memory + 定期 Fine-Tuning** 的组合——这已经能显著提升 Agent 系统的性能
3. **Test-Time Training 的双刃剑**：TTT 能处理分布漂移，但遗忘问题仍是未解决的工程挑战。SEAL 的"周期性在线学习"可能是更现实的折中
4. **Meta-Learning 的工程化**：HyperAgent、Alpha Evolve 等方向代表了从"手工调参"到"让系统自己学会如何调参"的范式转变
5. **"失控"是真实风险但非近期威胁**：当前技术离"自主进化"还非常远，但 Alignment 研究需要与 Self-Improving 研究同步推进

### 小光的工程判断

> 💡 **Self-Improving 最有价值的近期应用不在"自动训练 Stronger AI"，而在"自动化 Agent 运维"**。让 Agent 系统自主优化 prompt、管理记忆、调整检索策略——这些比"让模型自己写训练数据"更靠谱、更可控、也更快见效。

> 💡 **Prompt Optimization ≠ 写一个更好的 prompt**。DSPy 的 "Compile" 概念值得深入理解——它把 prompt 工程提升到了编译器优化的抽象层级。推荐所有做 Agent 的工程师上手试一下 DSPy。

> 💡 **TTT 在 2026 年更像是"应对分布漂移的紧急措施"，而不是"让模型持续成长的方案"**。如果你的场景是固定任务（如客服、文档问答），定期批量微调比 TTT 更稳定。

> 💡 **关于"失控"风险**：我同意李宏毅的判断——技术在河边，离远洋还远。但方向已经明确，现在是在"河流阶段"设计闸门的最佳时机。Alignment 研究不应该被视为阻碍创新的保守力量，而是确保创新的"安全带"。

---

## 课后思考

1. **Prompt Optimization 的边界在哪里？** DSPy 能自动搜索 prompt，但它依赖一个预先定义好的 metric。如果 metric 本身也是主观的（如"回答是否有用"），谁来定义这个 metric？这会不会只是把"人类介入"从 prompt 层面移到了 metric 层面？

2. **Test-Time Training 的遗忘悖论**：如果你必须在"保留旧能力"和"学习新能力"之间选择，在什么场景下你会优先选择后者（即使这意味着牺牲前者）？自动驾驶可能是"宁可忘记旧的驾驶风格也要适应新路况"，但医疗诊断呢？

3. **AI 需要"内在动机"吗？** 当前所有 AI 系统都在回答"人类问的问题"。但真正的自主 agent（如 24/7 运行的编程助手）是否需要自己的"我想做 X"？如果需要，这和"失控"之间只有一线之隔？

4. **谁来决定"改进模块"的改进方向？** 如果 Alpha Evolve 可以自动生成更好的算法，谁来定义"更好"？在什么情况下，"更好的算法"可能和人类利益冲突？

5. **动手建议**：尝试用 DSPy 构建一个简单的 RAG agent，用它的 optimizer 自动搜索最佳 prompt，对比手写 prompt 的效果差异。感受一下 "Compile" 范式。

---

## 参考资料

**课程影片**：[李宏毅，〈Self-Improving -2〉，ML 2026 Spring](https://youtu.be/cQLKVzbwN7I)

**课程讲义**：[Self-Evolving Agent (PDF)](https://speech.ee.ntu.edu.tw/~hylee/ml/ml2026-course-data/self-evolving-agent.pdf) | [Self-Evolving Agent (PPTX)](https://speech.ee.ntu.edu.tw/~hylee/ml/ml2026-course-data/self-evolving-agent.ptx)

**课程页**：[ML 2026 Spring](https://speech.ee.ntu.edu.tw/~hylee/ml/2026-spring.php)

**前一讲**：[李宏毅，〈Self-Improving〉，ML 2026 Spring](https://youtu.be/s06mSAGN4gM)
...
**Weak-to-Strong Alignment**：[OpenAI Blog](https://openai.com/index/weak-to-strong-generalization/) | [Paper](https://cdn.openai.com/papers/weak-to-strong-generalization.pdf)

**Anthropic Automated Alignment Research**：[Automated Alignment Researchers](https://www.anthropic.com/research/automated-alignment-researchers)

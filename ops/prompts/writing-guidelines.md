...
- Hexo 会根据 `##` 标题自动生成文章目录（TOC）。为避免目录冗余，正文中的 `##` / `###` 标题不得前置数字编号（如 `## 1. xxx`、`### 2.1 xxx`），应直接写成 `## xxx`、`### xxx`；仅在需要显式层次编号的极少数场景（如对比表格配套标题）才可使用。
- 参考资料中必须包含课程影片、课程讲义或 PPT、课程页；如果引用代码，也要把代码链接列入参考资料。
- 参考资料每一条的链接必须覆盖足够的信息范围，让读者直觉点击任意关键部分都能跳转。至少应把作者、标题、来源和年份做成一个连续的可点击链接，而不是只在末尾一个词上加链接（如 `[arXiv:xxx]`），导致读者需要精确点击某一个小词才能跳转。例如：推荐写成 `[Dao et al., "FlashAttention: Fast and Memory-Efficient Exact Attention with IO-Awareness", NeurIPS 2022](https://arxiv.org/abs/2205.14135)` 或 `[李宏毅，〈解剖小龙虾 — 以 OpenClaw 为例介绍 AI Agent 的运作原理〉，ML 2026 Spring](https://youtu.be/xxx)`，将作者、标题和来源一起包在链接内。
...
- `参考资料` 必须尽量使用 Markdown 链接形式，例如 `[Attention Is All You Need](https://arxiv.org/abs/1706.03762)` 或 `[官方文档](https://example.com/docs)`，方便读者直接跳转学习；不要只写裸 URL、纯 arXiv 编号或不可点击的来源名。
- 每个参考资料链接发布前必须核对 URL 正确性。优先链接到原始论文页面、官方文档、官方 GitHub、课程页或官方博客；如果某个旧链接会跳转到新路径，正文应使用最终可访问的新链接。

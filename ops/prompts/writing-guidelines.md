...
- 参考资料每一条的内容链接必须覆盖足够的信息范围，但加粗前缀（如 `**课程影片**：`）不包入链接。正确格式：`**课程影片**：[李宏毅，〈标题〉，ML 2026 Spring](https://youtu.be/xxx)` 或 `**Flash Attention**：[Dao et al., "FlashAttention: ...", NeurIPS 2022](https://arxiv.org/abs/xxx)`——加粗标签为纯文本，后面的作者、标题、来源整体为一个可点击链接。

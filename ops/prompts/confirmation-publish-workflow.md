...
- `TECH-...` 技术点讲解文章必须偏向“某一个关键技术点的研究级严谨综述”，能作为 AI 专业研究生学习该技术点的切入点；禁止写成热点拼盘、新闻评论或泛泛趋势总结。
- `TECH-...` 必须优先参考原论文、最初技术报告、官方源码、官方文档或作者/机构发布的一手材料；二手博客、媒体文章、社交媒体只能作为线索，不得作为核心事实依据。
- `TECH-...` 如果存在公开源码或官方实现，必须尽量讲清源码路径、核心函数/模块、关键张量形状或伪代码映射；如果没有可靠公开源码，必须明确说明。
...
- `TECH-...` 或任何含公式文章必须开启 `mathjax: true` front matter，公式必须写成 `$...$` 或 `$$...$$`，禁止把公式放进代码块或纯文本块。
...
- 至少 1 个原始核心来源：原论文、最初技术报告、官方 whitepaper、官方代码仓库或作者/机构一手说明。
- 优先补充 3 个以上一手/权威辅助来源：后续论文、官方实现、框架文档、权威课程或高质量工程文档。
- 二手资料只能用于发现线索或辅助理解，不计入核心事实来源。
- 如果核心来源不足，禁止按研究级 TECH 发布；应降级为技术札记、重新检索或暂停。
...
- 行内公式使用 `$...$`。
- 显示公式使用 `$$...$$`。
- 禁止用 fenced code block、普通反引号或纯文本块承载公式。
- 本地 `npx hexo generate` 后，必须检查目标 HTML 中存在 `mjx-container`，且文章目录中没有公式文本或 `$$` 片段。
...
1. 不要 commit。
2. 先修复公式渲染、行内数学、目录污染、来源或源码讲解问题。
3. 再次执行 `npx hexo generate` 和 `npm run validate:tech`。
4. 仍失败则写入错误日志并暂停。
